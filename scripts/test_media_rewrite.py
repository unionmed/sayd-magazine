...
def test_apply_does_not_drop_homepage_or_en_heroes() -> None:
    ...
    assert "suhail-2026-in-photos-falcons-visitors" not in en
    gallery = root / "docs" / "en" / "posts" / "suhail-2026-in-photos-falcons-visitors" / "index.html"
...
def test_visible_2022_articles_local_only() -> None:
    ...
    for slug in slugs:
        ...
        for src in srcs:
            if "media/" not in src:
                continue
            rel = src.split("media/", 1)[1].split("?", 1)[0]
            path = docs / "media" / rel
